"""Evaluation CLI — a separate entry point so nothing in `pagebirdy.cli` changes.

    python -m pagebirdy.eval jobs
    python -m pagebirdy.eval job <job_id> [--neural] [--mqm] [--gold docs/eval/gold.jsonl]
    python -m pagebirdy.eval pdf  <source.pdf>  <translated.pdf>
    python -m pagebirdy.eval idml <source.idml> <translated.idml> [--export-json e.json]
    python -m pagebirdy.eval gold [--out docs/eval/gold.jsonl] [--lang es]
...
from pagebirdy.eval import runner, scorecard
