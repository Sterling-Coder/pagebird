"""pagebirdy CLI.
    ...
    python -m pagebirdy.cli inspect  <pdf> [--pages 0,1]
    python -m pagebirdy.cli translate <pdf> --out out/ [--pages 0,1]
"""
...
from pagebirdy.pipeline import pdf_to_idml, translate_idml, translate_pdf
...
def cmd_pdf_to_idml(args) -> int:
    ...
    for note in report["notes"]:
        print(f"  - {note}")
    print("next       : python -m pagebirdy.cli translate-idml "
...
def cmd_ocr(args) -> int:
    ...
    from pagebirdy.ingest.ocr import (detect_image_regions, detect_scanned_pages,
...
def cmd_equations(args) -> int:
    ...
    from pagebirdy.protect.equations import active_engine, extract_equations
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="pagebirdy")
